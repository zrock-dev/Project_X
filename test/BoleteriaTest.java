...
class BoleteriaTest {
    ...
    public void testComprarBoletoSinCI(){
        boleteria.comprarBoletoSinCI(MetodoPago.TARJETA,"B","A", 2, Semana.MIERCOLES,ModoPresentacion.BIDIMENSIONAL);
        assertEquals(40,boleteria.getPrecioPelicula());

        boleteria.comprarBoletoSinCI(MetodoPago.TARJETA,"B","A", 3,Semana.JUEVES,ModoPresentacion.TRIDIMENSIONAL);
        assertEquals(132,boleteria.getPrecioPelicula());

        boleteria.comprarBoletoSinCI(MetodoPago.EFECTIVO,"B","A", 4,Semana.LUNES,ModoPresentacion.BIDIMENSIONAL);
